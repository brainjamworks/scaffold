...
import type { XBlockHandlerElement, XBlockRuntime } from "./api";
...
export function renderBlock(
  ...
  if (!(mountElement instanceof HTMLElement)) {
    throw new Error("Scaffold Studio mount element must be an HTMLElement.");
  }
  if (!data.innerUrl) {
    throw new Error("Scaffold Studio payload is missing innerUrl.");
  }

  runtime.element = handlerElement;
  applyStudioLayoutCompat(mountElement);
