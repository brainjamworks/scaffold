globalThis.ScaffoldStudentView = function ScaffoldStudentView(runtime, element, data) {
  (async function mountScaffoldStudentView() {
    var target = unwrapXBlockElement(element);

    try {
      runtime.element = element;
      if (!data || typeof data.outerUrl !== "string" || !data.outerUrl) {
        throw new Error("Scaffold student payload is missing outerUrl.");
      }
      var module = await import(data.outerUrl);
      module.renderBlock(target, data, runtime, element);
    } catch (error) {
      console.error("Failed to load Scaffold student view:", error);
      renderScaffoldLoadError(
        target,
        "Failed to load Scaffold content. Please refresh the page. If the problem persists, contact your instructor.",
      );
    }
  })();
};
